...
export default function App() {
  ...
  var fetchNews = useCallback(function() {
    ...
    var url = "";
    if (search) {
      url = "/api/news?search=" + encodeURIComponent(search);
    } else {
      url = "/api/news?category=" + cat + "&country=" + country.code;
    }
    fetch(url)
              ...
              {rest.map(function(article, idx) {
                return (
                  <NewsCard key={article.url + "-" + idx} article={article} T={T} dark={dark} view={view} idx={idx}
                    ...
                    liked={import { useState, useEffect, useRef, useCallback } from "react";
...
likes.includes(idx)} onLike={function() { toggleLike(idx); }}
                    shareOpen={shareId === idx} onShareToggle={function(e) { e.stopPropagation(); setShareId(shareId === idx ? null : idx); }}
                    onShare={handleShare}
                  />
                );
