...
function NewsCard({ article, T, dark, view, expanded, onExpand, bookmarked, onBookmark, liked, onLike, shareOpen, onShareToggle, onShare, idx }) {
  ...
  const fallback = "https://images.unsplash.com/photo-1504711434969-e33886168f5c?w=700&h=400&fit=crop";
  ...
  return (
    <article
      ...
      style={{
        ...
        boxShadow: hov ? "0 12px 36px rgba(0,0,0,0.13)" : "0 1px 4px rgba(0,0,0,0.07)",
    ...
    >
      <div style={{ position: "relative", flexShrink: 0, width: isList ? 200 : "100%", paddingTop: isList ? 0 : "56%", minHeight: isList ? 148 : 0, overflow: "hidden", background: T.border }}>
        {!imgOk && (
          <div style={{ position: "absolute", inset: 0, background: dark ? "#1e3048" : "#e2e8f0", animation: "pulse 1.2s infinite" }} />
        )}
        <img
          src={article.urlToImage || fallback}
          alt={article.title}
          onLoad={() => setImgOk(true)}
          onError={e => { e.target.src = fallback; setImgOk(true); }}
      ...
      <div style={{ padding: 14, display: "flex", flexDirection: "column", flex: 1 }}>
        ...
        <h3
          onClick={onExpand}
          style={{ fontSize: 14.5, fontWeight: 700, lineHeight: 1.35, color: T.text, marginBottom: 7, display: "-webkit-box", WebkitLineClamp: expanded ? 99 : 2, WebkitBoxOrient: "vertical", overflow: "hidden", cursor: "pointer" }}
        >
          {article.title}
        </h3>
        ...
        <button
          onClick={onExpand}
          style={{ alignSelf: "flex-start", background: "none", border: "none", color: T.accent, fontSize: 11, fontWeight: 700, padding: 0, marginBottom: 8, cursor: "pointer" }}
        >
          {expanded ? "▲ Less" : "▼ More"}
        </button>
        ...
        <div style={{ display: "flex", gap: 6 }}>
          <a
            ...
            style={{ flex: 1, padding: "9px 10px", background: "linear-gradient(135deg,#f97316,#ef4444)", color: "#fff", border: "none", borderRadius: 8, fontWeight: 700, fontSize: 12, cursor: "pointer", boxShadow: "0 4px 12px rgba(249,115,22,0.35)", textAlign: "center", textDecoration: "none", display: "flex", alignItems: "center", justifyContent: "center" }}
          >
            Read Full →
          </a>
          ...
          <button
            onClick={onLike}
            style={{ width: 34, height: 34, border: "1px solid " + (liked ? "#ef4444" : "rgba(239,68,68,0.3)"), background: liked ? "rgba(239,68,68,0.12)" : "transparent", borderRadius: 8, fontSize: 15, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer" }}
          >
            {liked ? "❤️" : "🤍"}
          </button>
          ...
          <div style={{ position: "relative" }} onClick={function(e) { e.stopPropagation(); }}>
            <button
              ...
              style={{ width: 34, height: 34, border: "1px solid " + (shareOpen ? T.accent : T.border), background: shareOpen ? T.accent + "18" : "transparent", borderRadius: 8, fontSize: 14, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", color: T.text }}
            >
              📤
            </button>
            {shareOpen && (
              <div style={{ position: "absolute", bottom: "calc(100% + 8px)", right: 0, background: T.surface, border: "1px solid " + T.border, borderRadius: 10, padding: 8, display: "flex", gap: 6, zIndex: 200, boxShadow: "0 12px 36px rgba(0,0,0,0.22)", animation: "scaleIn 0.2s ease" }}>
                {SHARES.map(function(p) {
                  return (
                    <button
                      key={p.name}
                      onClick={function() { onShare(p.name, article); }}
                      title={p.name}
          ...
          <button
            onClick={onBookmark}
            style={{ width: 34, height: 34, border: "1px solid " + (bookmarked ? T.accent : T.border), background: bookmarked ? T.accent + "15" : "transparent", borderRadius: 8, fontSize: 15, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", color: bookmarked ? T.accent : T.sub }}
          >
            {bookmarked ? "⭐" : "☆"}
          </button>
        </div>
...
export default function App() {
  ...
  const fetchNews = useCallback(function() {
    setLoading(true);
    setError(null);
    var url = "";
    if (search) {
      ...
      url = "https://newsapi.org/v2/top-headlines?country=" + country.code + "&category=" + cat + "&pageSize=20&apiKey=" + API_KEY;
    }
    fetch(url)
        ...
        <div style={{ maxWidth: 1200, margin: "0 auto", padding: "0 16px" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, height: 60 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
              ...
              <button onClick={fetchNews} title="Refresh" style={{ width: 36, height: 36, borderRadius: 9, border: "1px solid " + T.border, background: "transparent", color: T.text, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 15, cursor: "pointer" }}>🔄</button>
              <div style={{ position: "relative" }} onClick={function(e) { e.stopPropagation(); }}>
          ...
          {showSearch && (
            <form onSubmit={handleSearchSubmit} style={{ paddingBottom: 12, animation: "fadeUp 0.2s ease" }} onClick={function(e) { e.stopPropagation(); }}>
              <div style={{ position: "relative" }}>
                ...
                {(searchInput || search) && (
                  <button type="button" onClick={function() { setSearchInput(""); setSearch(""); }} style={{ position: "absolute", right: 52, top: "50%", transform: "translateY(-50%)", background: "none", border: "none", color: T.sub, fontSize: 14, cursor: "pointer" }}>✕</button>
                )}
              </div>
            </form>
          )}
